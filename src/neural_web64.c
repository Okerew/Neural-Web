...
void updateNeuronStates(Neuron *neurons, int num_neurons,
  ...
  for (int i = 0; i < num_neurons; i += 4) {
    ...
    new_states = _mm_add_ps(new_states, _mm_mul_ps(recurrent_inputs, recurrent_factor));
    new_states = _mm_add_ps(new_states, _mm_mul_ps(neighbor_influence, neighbor_factor));
...
void processNeurons(Neuron *neurons, int num_neurons, float *weights,
  ...
  for (int i = 0; i < num_neurons; i += 4) {
    ...
    for (int j = 0; j < max_connections; j++) {
      ...
      weighted_sum = _mm_add_ps(weighted_sum, _mm_mul_ps(weight_vector, target_state));
    }
    ...
    __m128 new_states = _mm_add_ps(
        _mm_mul_ps(current_states, decay_factor),
...
void advancedNeuronManagement(Neuron *neurons, int *connections,
                              float *weights, int *num_neurons,
...
void updateWeightsOnCPU(float* weights, const Neuron* neurons,
    ...
    for (int id = 0; id < max_neurons * max_connections; id++) {
        ...
        if (conn_idx >= neurons[neuron_idx].num_connections) continue;
...
void backpropagationOnCPU(const Neuron* neurons, float* weights,
                         ...
                         const int max_connections, const float* target_outputs,
                         float* output_errors, const float learning_rate) {
    for (int id = 0; id < max_neurons; id++) {
        // Step 1: Compute output error for the current neuron
        ...
        // Step 3: Calculate weight updates for all connections to this neuron
        for (int i = 0; i < max_connections; i++) {
            int conn_idx = id * max_connections + i;
            int connected_neuron = connections[conn_idx];
            
            if (connected_neuron >= max_neurons) continue;
            
            // Calculate gradient for this weight (backpropagation)
            float input_gradient = neurons[connected_neuron].output * error * activation_gradient;
            
            // Update weight using learning rate (gradient descent)
            float updated_weight = weights[conn_idx] - learning_rate * input_gradient;
            
            // Store updated weight back to the weights buffer
            weights[conn_idx] = updated_weight;
        }
    }
}
...
void processNeuronsOnCPU(Neuron* neurons, const float* weights,
    ...
    for (int id = 0; id < max_neurons; id++) {
        ...
        // Dynamic activation
        float dynamic_scale = ACTIVATION_SCALE * (1.0f + 0.1f * sinf(input_influence * M_PI));
...
int main() {
  ...
  for (int step = 0; step < STEPS; step++) {
    ...
    backpropagationOnCPU(neurons, weights, connections, max_neurons,
                         max_connections, target_outputs, outputErrors,
                         learning_rate);
    ...
    processNeuronsOnCPU(neurons, weights, connections, max_neurons, max_connections, input_tensor, input_size);                                      
