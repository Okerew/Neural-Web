                         ...
                         unsigned int step)
{
    ...
    for (unsigned int i = 0; i < working_memory->focus.capacity; i++)
    {
        ...
        {
            float error = std::fabs(neurons[i].output - target_outputs[i]);
            total_error += error;
    ...
    for (unsigned int i = 0; i < working_memory->active.size; i++)
    {
        ...
        if (activation < 0.1f)
        {
            std::memmove(&working_memory->active.entries[i],
                         &working_memory->active.entries[i + 1],
                         (working_memory->active.size - i - 1) * sizeof(WorkingMemoryEntry));
            working_memory->active.size--;
            i--;
        }
    }

    // Update clusters if focus is not empty
...
{
    GoalSystem *system = new GoalSystem(); // Allocate and zero-init struct

    system->goals = new Goal[capacity]; // Allocate typed array of Goal
                               ...
                               int current_step)
{
    ...
    for (int i = 0; i < MAX_NEURONS - 1;
         i += 3)
    { // Sample fewer pairs for efficiency
                         ...
                         float current_coherence)
{
    ...
    for (int i = 0; i < fmin(20, 100);
         i++)
    { // Look at more recent history (last 20)
...
SearchResults *performWebSearch(const char *query)
{
    ...
    response.data = (char *)malloc(1); // C++ requires a cast for malloc
    response.size = 0;
                       ...
                       float *observed_behavior, float *predicted_behavior)
{
    ...
    for (int i = 0; i < 5; i++)
    { // Assuming behavior vectors are of length 5
                       ...
                       float *other_goals, float *compromise)
{
    ...
    for (int i = 0; i < 5; i++)
    { // Assuming goal vectors are of length 5
    ...
    for (int i = 0; i < max_neurons; i++)
    {
        ...
        if (system->count < MAX_SPECIALIZED_NEURONS)
        {
            ...
            if (max_score >= system->specialization_threshold)
            {
                ...
                system->neurons[system->count].type = static_cast<NeuronSpecializationType>(spec_type);
